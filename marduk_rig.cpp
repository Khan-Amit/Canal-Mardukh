// ============================================================
// MARDUK — COMPLETE SYSTEM (Rig + Wallet + Index)
// ============================================================
// Compile: g++ -std=c++11 -pthread -O3 -o marduk marduk.cpp -lssl -lcrypto
// Run: ./marduk
...
// ============================================================
// STRATUM CLIENT (Wallet – submits to pool)
// ============================================================
...
// ============================================================
// RIG – Collects data, filters, hashes, sends to wallet
// ============================================================

string processData(const string& raw, int sock) {
    string binary = toBinary(raw);
    
    // Sluice-Bench: Check against reference database
    if (!hasXMRPattern(binary) && !hasBTCPattern(binary)) {
        cout << "🚫 REJECTED (no pattern match)\n";
        return "";
    }
    
    // Egg Shorter: Clean the data
    string washed = eggShorter(binary);
    if (washed.empty()) return "";
    
    // Generate HS256 (SHA-256)
    string hash = sha256(washed);
    
    // Wallet: Submit to pool
    if (sock >= 0) {
        ...
        poolSubmit(sock, hash, XSA_COUNTER.load());
        bool accepted = poolAccept(sock);
        if (accepted) {
            cout << "✅ WALLET: Accepted #" << TOTAL_SHARES.load() 
                 << " | Hash: " << hash.substr(0, 16) << "...\n";
        } else {
            cout << "❌ WALLET: Rejected #" << TOTAL_SHARES.load() << "\n";
        }
    }
...
// ============================================================
// WEB SERVER – Index (Wallet communicates with HTML)
// ============================================================
...
public:
    ...
    void start() {
        thread([&]() {
            ...
            listen(fd, 10);
            cout << "🌐 Index: http://127.0.0.1:8080\n";
...
.status{font-weight:bold;font-size:0.85rem;margin:10px 0;}
...
.wallet-display{background:#0a0a0a;border:1px solid #222;border-radius:8px;padding:12px 15px;margin:15px 0;word-break:break-all;font-family:'Courier New',monospace;font-size:0.75rem;color:#88ffaa;}
...
.dial-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;background:#0a0a0a;border:1px solid #222;border-radius:8px;padding:15px;margin:15px 0;}
...
.dial-stat .label{font-size:0.55rem;color:#888;text-transform:uppercase;}
.pool-selector{background:#0a0a0a;border:1px solid #222;border-radius:8px;padding:12px 15px;margin:15px 0;}
.pool-selector select{width:100%;padding:10px;background:#111;border:1px solid #333;border-radius:6px;color:#e0e0e0;font-size:0.85rem;outline:none;cursor:pointer;}
.log-window{background:#0a0a0a;border:1px solid #1a1a1a;border-radius:8px;padding:15px;min-height:300px;max-height:500px;overflow-y:auto;font-family:'Courier New',monospace;font-size:0.8rem;line-height:1.8;color:#aaa;margin:15px 0;}
...
button{background:transparent;color:#e0e0e0;border:1px solid #444;padding:10px 22px;border-radius:6px;font-size:0.8rem;cursor:pointer;flex:1;min-width:80px;}
...
<div class="container">
    ...
    <div class="log-window" id="logWindow">
        ...
        <div class="log-entry"><span class="time">⏳</span> <span class="error">⚠️ WALLET OFFLINE – Run ./marduk</span></div>
    </div>
    <div class="btn-group">
...
<script>
const WALLET_URL = "http://127.0.0.1:8080/status";
...
function addLog(msg,type){
    ...
    const c=type==='accepted'?'accepted':type==='rejected'?'rejected':type==='submitted'?'submitted':type==='deposited'?'deposited':type==='error'?'error':'info';
    d.innerHTML='<span class="time">'+t+'</span> <span class="'+c+'">'+msg+'</span>';
...
async function fetchRealData(){
    addLog('📡 Fetching from wallet...','info');
    try{
        ...
        if(poolResp==='accepted') addLog('✅ Wallet: Share accepted by pool','accepted');
        else if(poolResp==='rejected') addLog('❌ Wallet: Share rejected','rejected');
        addLog('📊 Shares: '+shares+' | Earned: '+earned.toFixed(8)+' XMR','accepted');
        setStatus(true); updateDisplay();
    }catch(e){
        setStatus(false);
        addLog('⚠️ WALLET OFFLINE: '+e.message,'error');
    }
}
...
document.getElementById('poolSelect').addEventListener('change',function(){
    ...
    addLog('🌐 Pool selected: '+selected,'info');
});
...
addLog('💡 Click FETCH REAL for wallet data.','info');
addLog('⚠️ Run ./marduk in terminal.','error');
updateDisplay();
setStatus(false);
...
// ============================================================
// RIG – Auto-generates data (simulates data flow)
// ============================================================
...
void autoGenerate(int sock) {
    for (int i = 0; true; i++) {
        processData(sampleData[i % sampleData.size()], sock);
        this_thread::sleep_for(chrono::seconds(2));
    }
}

// ============================================================
// MAIN – Runs autonomously
// ============================================================

int main() {
    cout << "\n════════════════════════════════════════════════════\n";
    cout << "⚖️ CANAL-MARDUKH™ — COMPLETE SYSTEM\n";
    cout << "════════════════════════════════════════════════════\n";
    cout << "📤 Wallet: " << WALLET << "\n";
    ...
    if (sock >= 0) {
        poolLogin(sock);
        cout << "✅ Wallet connected to pool\n";
    } else {
        cout << "⚠️ Wallet offline — no pool connection\n";
    }
    ...
    cout << "────────────────────────────────────────────────────\n";
    cout << "⛏️ Rig: Collecting data from canal...\n";
    cout << "🌐 Index: http://127.0.0.1:8080\n";
    cout << "────────────────────────────────────────────────────\n";
